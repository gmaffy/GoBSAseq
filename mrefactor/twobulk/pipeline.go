...
type SmoothedStats struct {
	...
	MeanHighBulkDP int
	MeanLowBulkDP  int
	SigSNPRatio    float64 // PyBSASeq-style fraction of Fisher-significant SNPs in window
...
func calcThresholds(highBulkDP, lowBulkDP int, highSmAF, lowSmAF float64, rep int) Thresholds {
	...
	for i := 0; i < rep; i++ {
		...
		gsArr[i] = math.Round(stats.GStatisticTwoBulk(int(hAlt), int(hRef), int(lAlt), int(lRef))*1e6) / 1e6
		edArr[i] = math.Round(stats.EuclideanDistance4(hSI, lSI)*1e6) / 1e6
		lodArr[i] = math.Round(stats.LODTwoBulk(int(hRef), int(hAlt), int(lRef), int(lAlt))*1e6) / 1e6
		bbArr[i] = math.Round(stats.BetaBinomialLogBFTwoBulk(int(hAlt), int(hRef), int(lAlt), int(lRef))*1e6) / 1e6
	}
...
func smoothChromosome(stats []BSAstats, windowSize int64, step int64, fisherAlpha float64) []SmoothedStats {
	...
	for center := minPos; center <= maxPos; center += step {
		...
		for _, s := range stats {
			...
			nSNPs++
			if fisherAlpha > 0 && s.FisherP <= fisherAlpha {
				sigSNPs++
			}
		...
		if nSNPs > 0 {
			sm.SigSNPRatio = float64(sigSNPs) / float64(nSNPs)
		}
...
func RunTwoBulkTwoParents(cfg utils.AnalysisConfig, hfCfg utils.HardFilterConfig) {
	...
	for chrom, stats := range chromStats {
		...
		smoothed := smoothChromosome(stats, windowSize, stepSize, cfg.FisherAlpha)
