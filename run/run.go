...
type BamInfo struct {
	Name string // logical name from GetBamName
	Path string // absolute path to BAM file
}

func bsaSeqTypeBam(cfg *utils.AnalysisConfig) (string, map[string]BamInfo, error) {
	bamMap := make(map[string]BamInfo)
	...
	if hp {
		hpBamName, err := GetBamName(hpBam)
		if err != nil {
			return "", nil, err
		}
		cfg.HighParentName = hpBamName
		bamMap["hp"] = BamInfo{Name: hpBamName, Path: hpBam}
	}
	if lp {
		lpBamName, err := GetBamName(lpBam)
		if err != nil {
			return "", nil, err
		}
		cfg.LowParentName = lpBamName
		bamMap["lp"] = BamInfo{Name: lpBamName, Path: lpBam}
	}
	if hb {
		hbBamName, err := GetBamName(hbBam)
		if err != nil {
			return "", nil, err
		}
		cfg.HighBulkName = hbBamName
		bamMap["hb"] = BamInfo{Name: hbBamName, Path: hbBam}
	}
	if lb {
		lbBamName, err := GetBamName(lbBam)
		if err != nil {
			return "", nil, err
		}
		cfg.LowBulkName = lbBamName
		bamMap["lb"] = BamInfo{Name: lbBamName, Path: lbBam}
	}

	switch {
	case hp && lp && hb && lb:
		return "2p2b", bamMap, nil
	case hp && lp && hb && !lb:
		return "2phb", bamMap, nil
	case hp && lp && !hb && lb:
		return "2plb", bamMap, nil
	case hp && !lp && hb && lb:
		return "hp2b", bamMap, nil
	case hp && !lp && hb && !lb:
		return "hphb", bamMap, nil
	case hp && !lp && !hb && lb:
		return "hplb", bamMap, nil
	case !hp && lp && hb && lb:
		return "lp2b", bamMap, nil
	case !hp && lp && hb && !lb:
		return "lphb", bamMap, nil
	case !hp && lp && !hb && lb:
		return "lplb", bamMap, nil
	case !hp && !lp && hb && lb:
		return "2b", bamMap, nil
	default:
		return "bad", nil, fmt.Errorf("invalid combination — at least one bulk is required")
	}
}
...
func filePathExists(theFile string) bool {
...
func GetBamName(bam string) (string, error) {
	if strings.HasSuffix(bam, ".bam") {
		...
		out, err := cmd.Output()
		if err != nil {
			return "", fmt.Errorf("samtools failed for %s: %w", bam, err)
		}
...
func Run(cfg *utils.AnalysisConfig, hf utils.HardFilterConfig) error {
	...
	if missing := missingGeneSpaceParams(cfg); len(missing) > 0 {
		...
		gsFiles := []string{cfg.Gff, cfg.GeneDesc, cfg.Prg}

		for _, gsFile := range gsFiles {
			if !filePathExists(gsFile) {
				fmt.Printf("Gene space analysis requires %s to be present\n", gsFile)
				return fmt.Errorf("gene space analysis requires %s to be present", gsFile)
			}
		}
	}
	...
	if runType == "bams" {
		...
		bType, bamMap, err := bsaSeqTypeBam(cfg)
		if err != nil {
			fmt.Println(err)
			return err
		}
		...
		bams := []string{}
		for _, bam := range bamMap {
			bams = append(bams, bam.Path)
		}
