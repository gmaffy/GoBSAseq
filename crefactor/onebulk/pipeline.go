...
func RunTwoParentsLowBulk(cfg utils.AnalysisConfig, hfcfg utils.HardFilterConfig) error {
	return runTwoParentsOneBulk(cfg, hfcfg, 2)
}

func RunTwoParentsHighBulk(cfg utils.AnalysisConfig, hfcfg utils.HardFilterConfig) error {
	return runTwoParentsOneBulk(cfg, hfcfg, 3)
}

func runTwoParentsOneBulk(cfg utils.AnalysisConfig, hfcfg utils.HardFilterConfig, bsaFilterType int) error {
	rdr := cfg.Rdr
	...
	nullAF := stats.ExpectedNullAF(cfg.Population, cfg.BCAltIsRecurrent)
	...
	passedVariants, original, hardFiltered, err := utils.HardFilterVcf(rdr, filteredVcfPath, badVcfPath, cfg, hfcfg, bsaFilterType)
	if err != nil {
		return fmt.Errorf("hard filter error: %w", err)
	}

	// Update indices to match subsetted samples: [HighParent, LowParent, Bulk].
	cfg.HighParentIdx = 0
	cfg.LowParentIdx = 1
	cfg.LowBulkIdx = 2
	cfg.HighBulkIdx = 2
	...
	for _, v := range passedVariants {
		...
		bulkSample := v.Samples[bulkIdx]

		bulkRefDep, _ := bulkSample.RefDepth()
		bulkAltDeps, _ := bulkSample.AltDepths()
		...
		s := OneBulkStats{
			...
			SI:               SI,
			AbsSI:            stats.AbsDeviationFromNull(SI, nullAF),
			ED:               stats.ED4OneBulk(SI, nullAF),
			Gstat:            math.Round(stats.GStatisticOneBulk(float64(bulkResAlleleCount), float64(bulkSusAlleleCount), nullAF)*1e6) / 1e6,
			LOD:              math.Round(stats.LodOneBulk(float64(bulkResAlleleCount), float64(bulkSusAlleleCount), nullAF)*1e6) / 1e6,
			BBLogBF:          math.Round(stats.BetaBinomialOneBulk(float64(bulkResAlleleCount), float64(bulkSusAlleleCount), nullAF)*1e6) / 1e6,
			Depth:            observedDepth,
		}
	...
	bulkSmAF := utils.SimulateAF(pop, float64(bulkSize), rep)
	popLevel := 0
	if pop == "F2" {
		popLevel = 1
	}
	...
	for chrom, stats := range chromStats {
		...
		allBRMBlocks = append(allBRMBlocks, calculateBRMBlocksOneBulk(chrom, smoothed, bulkSize, popLevel, nullAF, brmUAlpha)...)
	}
	...
	finalQTLs, err := GenerateHtmlPlotsAndQTLOneBulk(
		allSmoothed, bulkSmAF, bulkSize, pop, nullAF, cfg.BCAltIsRecurrent, cfg.Alphas, rep, outDir,
	)
	if err != nil {
...
func GenerateHtmlPlotsAndQTLOneBulk(
	...
	for _, chrom := range chroms {
		...
		chromBRMBlocks := calculateBRMBlocksOneBulk(chrom, stats, bulkSize, popLevel, expectedNullAF, brmUAlpha)
